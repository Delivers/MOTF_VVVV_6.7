...
@P EffectNodeBase.EffectNodeBaseShader: mixin FuseSDFBakeSDF_ComputeFX
***************************
****  ConstantBuffers  ****
***************************
cbuffer Globals [Size: 64]
@C    gridSize_id12 => FuseSDFUtils.gridSize
@C    gridCenter_id13 => FuseSDFUtils.gridCenter
@C    cellCount_id14 => FuseSDFUtils.cellCount
@C    cellSize_id15 => FuseSDFUtils.cellSize
@C    GridMargin_id16 => FuseSDFBakeSDF_ComputeFX.GridMargin
***************************
******  Resources    ******
***************************
@R    PositionBuffer_id17 => FuseSDFBakeSDF_ComputeFX.PositionBuffer [Stage: Compute, Slot: (0-0)]
@R    IndicesBuffer_id18 => FuseSDFBakeSDF_ComputeFX.IndicesBuffer [Stage: Compute, Slot: (1-1)]
@R    SDFBuffer_id11 => FuseSDFUtils.SDFBuffer [Stage: Compute, Slot: (0-0)]
@R    Globals => Globals [Stage: Compute, Slot: (0-0)]
***************************
*****     Sources     *****
***************************
@S    ComputeShaderBase => 8f045851eaa4513eb38a704d53889e6d
@S    FuseSDFBakeSDF_ComputeFX => 72c46e6a87063a427e005c7dddf0c52b
@S    FuseSDFUtils => d3bbf755cccaebce49ea043857d5a372
***************************
*****     Stages      *****
***************************
@G    Compute => f18a147fb12ded7f007b45170a3f2062
//
...
//   int GridMargin_id16;               // Offset:   48 Size:     4
//
// }
//
// Resource bind info for PositionBuffer_id17
// {
//
...
// }
//
// Resource bind info for IndicesBuffer_id18
// {
//
...
// ------------------------------ ---------- ------- ----------- -------------- ------
// PositionBuffer_id17               texture  struct         r/o             t0      1 
// IndicesBuffer_id18                texture  struct         r/o             t1      1 
...
//
#line 85 "C:\Users\sw\Documents\_projects\2024\MOTF\MOTF_VVVV_6.7\vvvv_gamma_6.7\log\shader_ComputeFXEffect_69a13f3c78aebda4e9f595b13722c135.hlsl"
...
#line 98
  iadd r3.xyz, r3.xyzx, -cb0[3].xxxx  // r3.x <- gridMin.x; r3.y <- gridMin.y; r3.z <- gridMin.z
...
#line 99
  iadd r4.xyz, r4.xyzx, cb0[3].xxxx  // r4.x <- gridMax.x; r4.y <- gridMax.y; r4.z <- gridMax.z
...
StructuredBuffer<float3> PositionBuffer_id17;
StructuredBuffer<int> IndicesBuffer_id18;
...
void Compute_id14(inout CS_STREAMS streams)
{
    ...
    uint numIndices, dum;
    IndicesBuffer_id18.GetDimensions(numIndices, dum);
    uint numTriangles = numIndices / 3;
    if (triIndex >= (int)numTriangles)
        return;
    uint index0 = IndicesBuffer_id18[triIndex * 3 + 0];
    uint index1 = IndicesBuffer_id18[triIndex * 3 + 1];
    uint index2 = IndicesBuffer_id18[triIndex * 3 + 2];
    float3 origin = gridCenter_id13 - (gridSize_id12 * .5);
    float3 tri0 = PositionBuffer_id17[index0] - origin;
    float3 tri1 = PositionBuffer_id17[index1] - origin;
    float3 tri2 = PositionBuffer_id17[index2] - origin;
    float3 aabbMin = min(tri0, min(tri1, tri2)) - cellSize_id15;
    float3 aabbMax = max(tri0, max(tri1, tri2)) + cellSize_id15;
    int3 gridMin = GetSdfCoordinates_id6(aabbMin) - GridMargin_id16;
    int3 gridMax = GetSdfCoordinates_id6(aabbMax) + GridMargin_id16;
