...
@P   - EffectNodeBase.EffectNodeBaseShader: mixin AddonBlurPass2_Internal_TextureFX [{Control = mixin DoFloat4 [{Before = mixin ComputeOrder [{Computes = [mixin AssignVarFloat4<Default_1> [{Value = InputFloat4<ShaderFX.InputValueFloat4,PerUpdate>}]]}]}, {Value = GetVarFloat4<Default_1>}]}, {Strength = mixin DoFloat [{Before = mixin ComputeOrder [{Computes = [mixin AssignVarFloat<Input_0> [{Value = InputFloat<ShaderFX.InputValueFloat,PerUpdate>}]]}]}, {Value = GetVarFloat<Input_0>}]}]
...
*****     Sources     *****
***************************
@S    AddonBlurPass2_Internal_TextureFX => 3817baa31a140f9491b28153c9b00e30
...
//
#line 245 "C:\Users\sw\Documents\_projects\2024\MOTF\MOTF_VVVV_6.7\vvvv_gamma_6.7\log\shader_TextureFXEffect_71259eed31b91bb8784ac49738a6bdc8.hlsl"
...
@G    Pixel => 9d3a909edd7b94a25fd65053fc8cc8f8
//
// Generated by Microsoft (R) HLSL Shader Compiler 10.1
//
//
...
//
#line 202 "C:\Users\sw\Documents\_projects\2024\MOTF\MOTF_VVVV_6.7\vvvv_gamma_6.7\log\shader_TextureFXEffect_71259eed31b91bb8784ac49738a6bdc8.hlsl"
...
{
    streams.Input_0_id101 = Compute_id83();
}
void Compute_id77(inout PS_STREAMS streams)
{
    streams.Default_1_id95 = Compute_id74();
}
float Lod_id29(float2 viewSize)
{
    return log2(max(viewSize.x, viewSize.y));
}
float Compute_id89(inout PS_STREAMS streams)
{
    return streams.Input_0_id101;
}
...
float4 Compute_id80(inout PS_STREAMS streams)
{
    return streams.Default_1_id95;
}
...
void ClacLodAndOffset_id69(float2 viewSize, float strength, out float lod, out float2 off)
{
    lod = strength * Lod_id29(viewSize);
    off = 0.5 / viewSize * pow(2, lod) * saturate(lod);
}
float Compute_id73(inout PS_STREAMS streams)
{
    Compute_id88(streams);
    return Compute_id89(streams);
}
float4 Compute_id72(inout PS_STREAMS streams)
{
    Compute_id79(streams);
    return Compute_id80(streams);
}
float4 Filter_id71(inout PS_STREAMS streams, float4 tex0col)
{
    float lod;
    float2 off;
    float strength = Compute_id73(streams);
    ClacLodAndOffset_id69(ViewSize_id77, strength, lod, off);
...
float4 Shading_id28(inout PS_STREAMS streams)
{
    ...
    return lerp(col, Filter_id71(streams, col), Compute_id72(streams));
}
